...
#define TEST_DEVICE_NAME "ATOUCH_BLUE"
...
int esp_bluetooth_send(uint8_t *buf,int len)
{
    if(is_connect == 1)
    {
        esp_ble_gatts_send_indicate(gl_profile_tab[PROFILE_APP_ID].gatts_if, conn_id, gl_profile_tab[PROFILE_APP_ID].char_handle,
                                    len, buf, false);
    }
